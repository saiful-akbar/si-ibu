function handleDelete(a) {
    bootbox.confirm({
        ...
        callback: (t) => {
            if (t) {
                const t = $("#form-delete-user");
                t.attr("action", `${main.baseUrl}/user/${a}`), t.submit();
            }
        },
    });
}
$(document).ready(function () {
    $("#avatar").change(function (a) {
        a.preventDefault();
        const { files: t } = $(this)[0];
        t && $("#avatar-view").attr("src", URL.createObjectURL(t[0]));
    }),
        $("button[type=reset]").click(function () {
            const a = $("#avatar-view");
            a.attr("src", a.data("src"));
        }),
        $(".menu-headers").change(function (a) {
            const t = $(this).is(":checked"),
                i = $(this).data("header");
            $(`.${i}`).attr("disabled", !t);
        }),
        $("#is-disable-password").change(function (a) {
            const t = $(this).is(":checked");
            $("#password").attr("disabled", !t);
        });
});
